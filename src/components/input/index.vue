<template>
  ...
  >
    ...
    <div
      ...
      :class="placeholderAlign ? `vux-x-input-placeholder-${placeholderAlign}` : ''"
    >
      <input
        :id="`vux-x-input-${uuid}`"
        v-if="!type || type === 'text'"
    ...
    <div class="weui-cell__ft">
      <icon
        ...
        v-show="!hasRightFullHeightSlot && !equalWith && showClear && currentValue !== '' && !readonly && !disabled && isFocus"
        @click.native="clear"
      ></icon>
      <icon
        name="input-warn"
      ...
      ></icon>
      <icon name="input-warn" class="vux-input-icon" v-show="novalidate && iconType === 'error'"></icon>
...
export default {
  ...
  methods: {
    ...
    maskValue(val) {
      const val1 = this.mask ? VMasker.toPattern(val, this.mask) : val;
      return val1;
    },
    reset(value = "") {
      this.dirty = false;
...
@import "../../style/weui/widget/weui-cell/weui-form/weui-vcode";
